...
from ..utils.gen_utils import get_kNN,get_edge_list,get_igraph_from_knn_list
...
def cutoffs(values, eps=1/400, mass=0.99, win=50, rel_slope=0.01):
    ...
    x = np.arange(len(v))
    p1, p2 = np.array([0, v[0]]), np.array([len(v)-1, v[-1]])
    num = np.abs(np.cross(p2 - p1, np.vstack([x, v]).T - p1))
    den = np.linalg.norm(p2 - p1)
    knee = int(np.argmax(num / den))
    out["knee"] = knee
...
def stable_rank(self,layer_extraction_params=None):
    ...
    #Initiate parameters
    core_fraction=layer_extraction_params.get('core_fraction',0.15)
    ...
    core_nodes=np.array(sorted_nodes[:int(cuts[core_selection_mode])]).astype(int)
    ...
    #ToDo: This part needs to be polished.
    if fine_grained:
        ...
        G_stable=get_igraph_from_knn_list(knn_list_stable,n1)
        ...
        labels_init = cluster_subset(G_stable, stable_new_core, res=res_t, densify=densify, adj_list=knn_list_n)
